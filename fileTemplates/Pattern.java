 ...
 *
 * <blockquote><pre>
 * ${NAME} p = ${NAME}.{@link #compile compile}("a*b");
 ...
 *
 * <blockquote><pre>
 * boolean b = ${NAME}.matches("a*b", "aaaaab");</pre></blockquote>
 *
 ...
 * <p>
 * The script names supported by <code>${NAME}</code> are the valid script names
 ...
 * <p>
 * The block names supported by <code>${NAME}</code> are the valid block names
 * accepted and defined by
 ...
 * {@code IsAlphabetic}. The supported binary properties by <code>${NAME}</code>
 * are
 * <ul>
 *   <li> Alphabetic
 *   <li> Ideographic
 ...
 *
 * <p>The <code>${NAME}</code> engine performs traditional NFA-based matching
    ...
    implements java.io.Serializable
{
     ...
     * <pre>
     * RegExp r1 = RegExp.compile("abc", ${NAME}.I|${NAME}.M);
    ...
    /* ${NAME} has only two serialized components: The pattern string
    ...
    /**
     * Boolean indicating this ${NAME} is compiled; this is necessary in order
     * to lazily compile deserialized ${NAME}s.
     */
     ...
     */
    private transient String normalized${NAME};
    ...
    /**
     * The number of capturing groups in this ${NAME}. Used by matchers to
     ...
     *
     * @throws  ${NAME}SyntaxException
     *          If the expression's syntax is invalid
     */
    public static ${NAME} compile(String regex) {
        return new ${NAME}(regex, 0);
    }
     ...
     *          If the expression's syntax is invalid
     */
    public static ${NAME} compile(String regex, int flags) {
        return new ${NAME}(regex, flags);
    }
     ...
     *
     * <blockquote><pre>
     * ${NAME}.matches(regex, input);</pre></blockquote>
     *
     * behaves in exactly the same way as the expression
     *
     * <blockquote><pre>
     * ${NAME}.compile(regex).matcher(input).matches()</pre></blockquote>
     *
     ...
     * create a <code>${NAME}</code> that would match the string
    ...
    /**
     * Recompile the ${NAME} instance from a stream.  The original pattern
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        // if length > 0, the ${NAME} is lazily compiled
        compiled = false;
        if (pattern.length() == 0) {
    ...
    /**
     * This private constructor is used to create all ${NAME}s. The pattern
     * string and match flags are all that is needed to completely describe
     * a ${NAME}. An empty pattern string results in an object tree with
     * only a Start node and a LastNode node.
     */
    private ${NAME}(String p, int f) {
        pattern = p;
        flags = f;
    ...
    private void normalize() {
        ...
        normalized${NAME} = Normalizer.normalize(pattern, Normalizer.Form.NFD);
        patternLength = normalized${NAME}.length();

        // Modify pattern to match canonical equivalences
        StringBuilder new${NAME} = new StringBuilder(patternLength);
        for(int i=0; i<patternLength; ) {
            int c = normalized${NAME}.codePointAt(i);
            ...
            if ((Character.getType(c) == Character.NON_SPACING_MARK)
                ...
                while(Character.getType(c) == Character.NON_SPACING_MARK) {
                    ...
                    if (i >= patternLength)
                        break;
                    c = normalized${NAME}.codePointAt(i);
                    sequenceBuffer.appendCodePoint(c);
                }
                String ea = produceEquivalentAlternation(
                                               sequenceBuffer.toString());
                new${NAME}.setLength(new${NAME}.length()-Character.charCount(lastCodePoint));
                new${NAME}.append("(?:").append(ea).append(")");
            } else if (c == '[' && lastCodePoint != '\\') {
                i = normalizeCharClass(new${NAME}, i);
            } else {
                new${NAME}.appendCodePoint(c);
            }
            lastCodePoint = c;
            i += Character.charCount(c);
        }
        normalized${NAME} = new${NAME}.toString();
    }
     ...
     */
    private int normalizeCharClass(StringBuilder new${NAME}, int i) {
...
loop:   for(int x=0, offset=0; x<nCodePoints; x++, offset+=len) {
    ...
    private void compile() {
        ...
        for (int x = 0; x < patternLength; x += Character.charCount(c)) {
            c = normalized${NAME}.codePointAt(x);
    ...
    /**
     * Used to print out a subtree of the ${NAME} to help with debugging.
     */
     ...
     */
    private ${NAME}SyntaxException error(String s) {
        return new ${NAME}SyntaxException(s, normalized${NAME},  cursor - 1);
    }
     ...
     * is made of individual elements that handle constructs in the ${NAME}.
